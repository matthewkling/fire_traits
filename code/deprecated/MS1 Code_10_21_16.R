...
traits_of_interest_cors <- 
      c("log_Bark.thickness","Plant.height","Self.pruning",
        "Flame_ht","Pct_consumed", "log_Flame_duration")
...
#Since flame height and percent consumed are tightly correlated, calculate the first principal component of their ordination, 
...
traits_of_interest <- 
      c("Bark.Thickness.25.4.FOFEM2017","Plant.height","Self.pruning",
        "Flame_ht","Pct_consumed","Flame_duration")

d$bt.pct <- 
      (d$Bark.Thickness.25.4.FOFEM2017-min(d$Bark.Thickness.25.4.FOFEM2017)) / 
      diff(range(d$Bark.Thickness.25.4.FOFEM2017))
d$ph.pct <- 
      (d$Plant.height-min(d$Plant.height)) / 
      diff(range(d$Plant.height))
d$sp.pct <- 
      (d$Self.pruning-min(d$Self.pruning)) / 
      diff(range(d$Self.pruning))
d$fh.pct <- 
      (d$Flame_ht-min(d$Flame_ht)) / 
      diff(range(d$Flame_ht))
d$pc.pct <- 
      (d$Pct_consumed -min(d$Pct_consumed)) / 
      diff(range(d$Pct_consumed))
d$fh_pc.pct <- #Need "1-x" because most negative PC scores are tallest flame lengths,
      #with largest percent consumed. **Leaning towards this one**
      1-(PC1-min(PC1)) / 
      diff(range(PC1))
d$fd.pct <- #Need "1-x" because most resistant duration is shortest.
      1-(d$log_Flame_duration-min(d$log_Flame_duration)) / 
      diff(range(d$log_Flame_duration)) 

#Apply (across each row) the weighted mean of the traits of interest, 
#(formerly weighted by trait completeness, but now have full dataset.
percentiles_of_interest <-
      c("bt.pct","ph.pct","sp.pct","fh.pct","pc.pct", "fd.pct")
percentiles_of_interest <-
...
ggsave("figures/MS1/Fig1_frs_ranking.png", 
